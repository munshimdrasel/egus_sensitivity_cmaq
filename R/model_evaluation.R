...
# ================================================================
#first nationwide evaluation for whole year
# ================================================================
datalist = list()
...
for (i in 1:length(daily.aqs)) {
  ...
  # say spin up period 4 days
  # aqs <- aqs %>% filter(Time_On >="2020-11-01")  ##YYY-dd-mmm
  aqs.var.tot <- aqs %>% dplyr::select(Time_On,  PM25_TOT_ob,PM25_TOT_mod, PM25_SO4_ob, PM25_SO4_mod,
                                       PM25_NO3_ob, PM25_NO3_mod, PM25_NH4_ob, PM25_NH4_mod, 
                                       PM25_OC_ob, PM25_OC_mod, PM25_EC_ob, PM25_EC_mod)
  aqs.var.tot$PM25_TOT_ob <- as.numeric(aqs.var.tot$PM25_TOT_ob)
  aqs.var.tot$PM25_TOT_mod <- as.numeric(aqs.var.tot$PM25_TOT_mod)
  ...
  # say spin up period 4 days
  # aqs <- aqs %>% filter(Time_On >="2020-11-01")  ##YYY-dd-mmm
  o3.aqs.var.tot <- o3.aqs %>% dplyr::select(Time_On,  O3_8hrmax_ob, O3_8hrmax_mod)
  o3.aqs.var.tot$O3_8hrmax_ob <- as.numeric(o3.aqs.var.tot$O3_8hrmax_ob)
  o3.aqs.var.tot$O3_8hrmax_mod<- as.numeric(o3.aqs.var.tot$O3_8hrmax_mod)
  ...
  combined.var <- merge(var_daily, o3.var_daily, by= c("Time_On"))
  
  #selecting only PM2.5 and O3 8hr max
  combined.var <- combined.var %>% dplyr::select(Time_On, PM25_TOT_ob, PM25_TOT_mod,
                                                 PM25_SO4_ob, PM25_SO4_mod,
                                                 PM25_NO3_ob, PM25_NO3_mod,
                                                 PM25_NH4_mod, PM25_NH4_ob)
  
  species.melt <- melt(combined.var, 
                       measure.vars = c("PM25_TOT_ob", "PM25_TOT_mod", 
                                        "PM25_SO4_ob", "PM25_SO4_mod",
                                        "PM25_NO3_ob", "PM25_NO3_mod",
                                        "PM25_NH4_mod", "PM25_NH4_ob"),
                       variable.name = "pollutants", value.name = "value")
...
nation_eval_all = do.call(rbind, datalist)


pm.spc.data_pm_tot <- nation_eval_all  %>% filter(pollutants %in% c("PM25_TOT_ob", "PM25_TOT_mod"))
  

#total PM2.5 nationwide

pm.spc.data_pm_tot <- reshape(pm.spc.data_pm_tot, idvar = "Time_On", timevar = "pollutants", direction = "wide")
  
  names(pm.spc.data_pm_tot)[names(pm.spc.data_pm_tot) == "value.PM25_TOT_ob"] <- 'PM25_TOT_ob'
  names(pm.spc.data_pm_tot)[names(pm.spc.data_pm_tot) == "value.PM25_TOT_mod"] <- 'PM25_TOT_mod'
  names(pm.spc.data_pm_tot)[names(pm.spc.data_pm_tot) == "Time_On"] <- 'date'
  
  mod.eval.pm25 <- as.data.table(modStats(pm.spc.data_pm_tot , mod = "PM25_TOT_mod", obs = "PM25_TOT_ob"))
  
  mod.eval.pm25$variable <- "pm25"
  
  
  # # ======Sulfate
  pm.spc.data_so4 <- nation_eval_all%>% filter(pollutants %in% c("PM25_SO4_ob", "PM25_SO4_mod")) %>% na.omit()

  # na.omit( pm.spc.data_so4) %>%
  ...
  # ggsave(paste0("pm.so4_eval_epa_region_", regions.vec[i],".png"), path = "./plots/", width=8, height=6, units="in")
   ...
   pm.spc.data_so4 <- reshape( pm.spc.data_so4, idvar = "Time_On", timevar = "pollutants", direction = "wide")

  names( pm.spc.data_so4)[names( pm.spc.data_so4) == "value.PM25_SO4_ob"] <- 'PM25_SO4_ob'
  names( pm.spc.data_so4)[names( pm.spc.data_so4) == "value.PM25_SO4_mod"] <- 'PM25_SO4_mod'
  names( pm.spc.data_so4)[names( pm.spc.data_so4) == "Time_On"] <- 'date'

  mod.eval.pm25.so4 <- as.data.table(modStats( pm.spc.data_so4 , mod = "PM25_SO4_mod", obs = "PM25_SO4_ob"))
  
  mod.eval.pm25.so4$variable <- "so4"
  #
  #
  # # ======nitrate
   pm.spc.data_no3 <- nation_eval_all %>% filter(pollutants %in% c("PM25_NO3_ob", "PM25_NO3_mod"))

  # na.omit( pm.spc.data_no3) %>%
  #   ggplot(aes(x=Time_On)) + geom_point(aes(y=value, color=pollutants), size=3) +
  #   geom_line(aes(y=value, color=pollutants))+
  ...
  pm.spc.data_no3 <- reshape( pm.spc.data_no3, idvar = "Time_On", timevar = "pollutants", direction = "wide")

  names( pm.spc.data_no3)[names( pm.spc.data_no3) == "value.PM25_NO3_ob"] <- 'PM25_NO3_ob'
  names( pm.spc.data_no3)[names( pm.spc.data_no3) == "value.PM25_NO3_mod"] <- 'PM25_NO3_mod'
  names( pm.spc.data_no3)[names( pm.spc.data_no3) == "Time_On"] <- 'date'

  mod.eval.pm25.no3 <- as.data.table(modStats( pm.spc.data_no3 , mod = "PM25_NO3_mod", obs = "PM25_NO3_ob"))
  ...
  # ======NH4
   pm.spc.data_nh4 <-nation_eval_all %>% filter(pollutants %in% c("PM25_NH4_ob", "PM25_NH4_mod"))

  # na.omit( pm.spc.data_nh4) %>%
  #   ggplot(aes(x=Time_On)) + geom_point(aes(y=value, color=pollutants), size=3) +
  #   geom_line(aes(y=value, color=pollutants))+
  ...
  # ggsave(paste0("PM.NH4_eval_epa_region_", regions.vec[i],".png"), path = "./plots/", width=8, height=6, units="in")
   ...
   pm.spc.data_nh4 <- reshape( pm.spc.data_nh4, idvar = "Time_On", timevar = "pollutants", direction = "wide")

  names( pm.spc.data_nh4)[names( pm.spc.data_nh4) == "value.PM25_NH4_ob"] <- 'PM25_NH4_ob'
  names( pm.spc.data_nh4)[names( pm.spc.data_nh4) == "value.PM25_NH4_mod"] <- 'PM25_NH4_mod'
  names( pm.spc.data_nh4)[names( pm.spc.data_nh4) == "Time_On"] <- 'date'

  mod.eval.pm25.nh4 <- as.data.table(modStats( pm.spc.data_nh4 , mod = "PM25_NH4_mod", obs = "PM25_NH4_ob"))
  ...
  eval.metrics <- rbind(mod.eval.pm25)
...
#plots
ev.metrics <- eval.metrics %>% filter(!variable %in% "o3")

#NMGE
ev.metrics %>%  ggplot(aes(x=season, y=NMGE)) + geom_point() +
  ...
  geom_hline(yintercept = 50, color = "blue", size=0.8) +
...
ev.metrics %>%  ggplot(aes(x=season, y=NMB)) + geom_point()+
  labs (title=paste0(("Normalized mean bias (%)")))  +
  ...
  theme (axis.title.x = element_blank(),
         ...
         title=element_text(size=20),
         strip.text.x = element_text(size = 20)) + 
  geom_hline(yintercept = 30, color = "blue", size=0.8) +
  geom_hline(yintercept = -30, color = "blue", size=0.8) +
  facet_wrap(epa_region~. )

ggsave(paste0("nmb.png"), path = "./plots/", width=8, height=6, units="in")



#Pearson R
ev.metrics %>%  ggplot(aes(x=season, y=r)) + geom_point()+
  ...
  theme (axis.title.x = element_blank(),
         ...
         title=element_text(size=20),
         strip.text.x = element_text(size = 20)) +
  geom_hline(yintercept = 0.4, color = "blue", size=0.8) +
  facet_wrap(epa_region~. )

ggsave(paste0("pearson_r.png"), path = "./plots/", width=8, height=6, units="in")
...
# =====================Region wise and season wise evaluation==============================
...
#plots
ev.metrics <- ev.metrics %>% filter(!variable %in% "o3")
...
#RMSE
ev.metrics %>%  ggplot(aes(x=season, y=RMSE)) + geom_point() +
  labs (title=expression(paste("RMSE, " , mu,"g/", m^3)))  +
...
ggsave(paste0("RMSE.png"), path = "./plots/", width=8, height=6, units="in")
