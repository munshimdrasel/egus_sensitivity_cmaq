...
#loading monthly gridded data
load (file=paste0("./data/hr2day_aconc/aconc_variables.RData"))

dt <- do.call(rbind,datalist)

dt_wide <- spread(dt, variable, z)

save(dt_wide, file=paste0("./data/hr2day_aconc/aconc_variables_wide.RData"))

load(file=paste0("./data/hr2day_aconc/aconc_variables_wide.RData"))

dt_wide <- dt_wide %>% mutate(EC_fraction= PM25_EC_AVG/PM25_TOT_AVG,
                              OC_fraction= PM25_OC_AVG/PM25_TOT_AVG,
                              NH4_fraction= PM25_NH4_AVG/PM25_TOT_AVG,
                              NO3_fraction= PM25_NO3_AVG/PM25_TOT_AVG,
                              SO4_fraction= PM25_SO4_AVG/PM25_TOT_AVG)
...
#getting us bounday converting crs to commonly used CRS in cmaq
us.sf <-  USAboundaries::us_states()  %>%  filter(!stusps %in% c("PR", "HI", "AK") ) %>%st_transform( crs = p4s)

plot.dt <- as.data.table( dt_wide[, c('PM25_TOT_AVG', 'PM25_SO4_AVG', 'SO4_fraction')])
plot( plot.dt[, .( PM25_TOT_AVG, PM25_SO4_AVG, SO4_fraction)])

#plotting PM2.5 SO2 EGU sensitivity data from Coal Fueled EGUs
plot_so4<- ggplot( ) +
  geom_sf( data = us.sf, size = 1) +
  geom_sf(data=dt_wide, aes( fill = SO4_fraction ,  geometry = geometry), color = NA) +
...
ggsave("so4_pm2.5_fraction.png", path = "./plots/", width=10, height=7, units="in")

plot_no3<- ggplot( ) +
  geom_sf( data = us.sf, size = 1) +
  geom_sf(data=dt_wide, aes( fill = NO3_fraction ,  geometry = geometry), color = NA) +
  scale_fill_viridis_c( option="plasma", name = (paste(  "NO3 fraction")),
                        limit=c(0,NA), oob = scales::squish) +
  scale_x_continuous( expand = c( 0, 0)) +
  scale_y_continuous( expand = c( 0, 0)) +
  theme_bw() +
...
ggsave("no3_pm2.5_fraction.png", path = "./plots/", width=8, height=7, units="in")

plot_nh4<- ggplot( ) +
  geom_sf( data = us.sf, size = 1) +
  geom_sf(data=dt_wide, aes( fill = NH4_fraction ,  geometry = geometry), color = NA) +
  scale_fill_viridis_c( option="plasma", name = expression(paste(  "NH4 fraction")),
                        limit=c(0,NA), oob = scales::squish) +
  scale_x_continuous( expand = c( 0, 0)) +
  scale_y_continuous( expand = c( 0, 0)) +
  theme_bw() +
...
ggsave("nh4_pm2.5_fraction.png", path = "./plots/", width=8, height=7, units="in")


plot_EC<- ggplot( ) +
  geom_sf( data = us.sf, size = 1) +
  geom_sf(data=dt_wide, aes( fill = EC_fraction ,  geometry = geometry), color = NA) +
  scale_fill_viridis_c( option="plasma", name = expression(paste(  "EC fraction")),
                        limit=c(0,NA), oob = scales::squish) +
  scale_x_continuous( expand = c( 0, 0)) +
  scale_y_continuous( expand = c( 0, 0)) +
  theme_bw() +
...
ggsave("EC_pm2.5_fraction.png", path = "./plots/", width=8, height=7, units="in")


plot_oc<- ggplot( ) +
  geom_sf( data = us.sf, size = 1) +
  geom_sf(data=dt_wide, aes( fill = OC_fraction ,  geometry = geometry), color = NA) +
  scale_fill_viridis_c( option="plasma", name = expression(paste(  "OC fraction")),
                        limit=c(0,NA), oob = scales::squish) +
  scale_x_continuous( expand = c( 0, 0)) +
  scale_y_continuous( expand = c( 0, 0)) +
  theme_bw() +
...
ggsave("oc_pm2.5_fraction.png", path = "./plots/", width=8, height=7, units="in")


plot_pm2.5<- ggplot( ) +
  geom_sf( data = us.sf, size = 1) +
  geom_sf(data=dt_wide, aes( fill = PM25_TOT_AVG ,  geometry = geometry), color = NA) +
  scale_fill_viridis_c( option="plasma", name = expression(paste( PM[2.5], " ug/", m^3)),
                        limit=c(0,NA), oob = scales::squish) +
  scale_x_continuous( expand = c( 0, 0)) +
  scale_y_continuous( expand = c( 0, 0)) +
  theme_bw() +
...
ggsave("pm25.png", path = "./plots/", width=8, height=7, units="in")
